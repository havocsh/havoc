...
resource "aws_s3_bucket" "listener_lb_logs" {
  bucket = "${var.deployment_name}-listener-lb-logs"

  tags = {
    Name = "${var.deployment_name}-listener-lb-logs"
  }
}

resource "aws_s3_bucket_acl" "listener_lb_logs" {
  bucket = aws_s3_bucket.listener_lb_logs.id
  acl    = "private"
}

resource "aws_s3_bucket_public_access_block" "listener_lb_logs" {
  bucket                  = aws_s3_bucket.listener_lb_logs.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
